...
public class fcliente {
    ...
    public DefaultTableModel mostrar(String buscar) {
        ...
        String[] titulos = {"ID", "Nombre", "Apaterno", "Amaterno", "Doc", "Número Documento", "Dirección", "Teléfono", "Email", "Estado"};
        ...
        sSQL = "select p.idpersona,p.nombre,p.apaterno,p.amaterno,p.tipo_documento,p.num_documento,"
                + "p.direccion,p.telefono,p.email,c.estado from persona p inner join cliente c "
                + "on p.idpersona=c.idpersona where num_documento like '%"
                + buscar + "%' order by idpersona desc limit 0,100";
    ...
    public boolean insertar(vcliente dts) {
        sSQL = "insert into persona (nombre,apaterno,amaterno,tipo_documento,num_documento,direccion,telefono,email)"
                + "values (?,?,?,?,?,?,?,?)";
        sSQL2 = "insert into cliente (idpersona,estado)"
    ...
    public boolean eliminar2(vcliente dts, String estado) {
        if (estado.equalsIgnoreCase("A")) {
           sSQL = "update cliente set estado='D'"
                + " where idpersona=?"; 
        }else if (estado.equalsIgnoreCase("D")){
           sSQL = "update cliente set estado='A'"
                + " where idpersona=?";  
        }
